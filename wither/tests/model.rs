...
async fn model_find_should_find_all_instances_of_model_with_no_filter_or_options() {
    ...
    user.save(db.clone(), None).await.expect("Expected a successful save operation.");

    let mut users_from_db: Vec<_> = User::find(db.clone(), None, None)
        .await
...
async fn model_find_should_find_instances_of_model_matching_filter() {
    ...
    let mut users_from_db: Vec<_> = User::find(db.clone(), Some(doc), None)
...
async fn model_find_one_and_delete_should_delete_the_target_doc() {
    ...
    user.save(db.clone(), None).await.expect("Expected a successful save operation.");
    user2.save(db.clone(), None).await.expect("Expected a successful save operation.");
...
async fn model_save_should_save_model_instance_and_add_id() {
    ...
    let precount = User::collection(db.clone()).count_documents(None, None).await.unwrap();
    user.save(db.clone(), None).await.expect("Expected a successful save operation.");
    let postcount = User::collection(db.clone()).count_documents(None, None).await.unwrap();
...
async fn model_update_should_noop_where_filter_selects_on_nonextant_field() {
    ...
    let res = user.update(db.clone(), filter_doc, update_doc, None).await;
...
async fn model_delete_should_delete_model_instance() {
    ...
    let presave = User::collection(db.clone()).count_documents(None, None).await.unwrap();
    user.save(db.clone(), None).await.expect("Expected a successful save operation.");
    let postsave = User::collection(db.clone()).count_documents(None, None).await.unwrap();
    user.delete(db.clone()).await.unwrap();
    let postdelete = User::collection(db.clone()).count_documents(None, None).await.unwrap();
...
//     let _ = User::sync(db.clone()).await.expect("Expected a successful sync operation.");
...
//     let _ = DerivedModel::sync(db.clone()).await.expect("Expected a successful sync operation.");
//     let mut output_indices: Vec<Document> = coll.list_indexes()
//         .await.expect("Expected to successfully open indices cursor post-test.")
//         .filter_map(|doc_res| doc_res.ok())
//         .collect();
...
//     let _ = Derived2dModel::sync(db.clone()).await.expect("Expected a successful sync operation.");
//     let mut output_indices: Vec<Document> = coll.list_indexes()
//         .await.expect("Expected to successfully open indices cursor post-test.")
//         .filter_map(|doc_res| doc_res.ok())
//         .collect();
...
//     let _ = Derived2dsphereModel::sync(db.clone()).await.expect("Expected a successful sync operation.");
//     let mut output_indices: Vec<Document> = coll.list_indexes()
//         .await.expect("Expected to successfully open indices cursor post-test.")
//         .filter_map(|doc_res| doc_res.ok())
//         .collect();
...
// //     let _ = DerivedGeoHaystackModel::sync(db.clone()).await.expect("Expected a successful sync operation.");
// //     let mut output_indices: Vec<Document> = coll.list_indexes()
// //         .await.expect("Expected to successfully open indices cursor post-test.")
// //         .filter_map(|doc_res| doc_res.ok())
// //         .collect();
...
//     new_user.save(db.clone(), None).await.expect("Expected to successfully save new user instance.");

//     let _ = User::migrate(db.clone()).await.expect("Expected a successful migration operation.");
